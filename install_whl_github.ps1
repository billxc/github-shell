...
if (-not (Get-Command uv -ErrorAction SilentlyContinue)) {
    # Ensure uv is installed
    Write-Host "uv is not installed. Install it using Scoop"
    
    if (-not (Get-Command scoop -ErrorAction SilentlyContinue)) {
        Write-Host "Scoop is not installed. Install Scoop first." -ForegroundColor Red
        powershell -ExecutionPolicy ByPass -c "irm https://get.scoop.sh | iex"
    }
    scoop install python uv
}
...
# $downloadedFile = Download-RepoFile -GitHubToken $token -FilePath "README.md"
# $downloadedFile = Download-RepoFile -GitHubToken $token -FilePath "src/main.py" -OutputPath "./downloaded/main.py"
# $downloadedFile = Download-RepoFile -GitHubToken $token -FilePath "config.json" -Branch "develop"
